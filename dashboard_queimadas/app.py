...
import requests, warnings, unicodedata, re, json
import ee
...
try:
    ...
    credentials = ee.ServiceAccountCredentials(
        ...
        # FIX 11: garante que key_data é sempre uma string JSON válida
...
def add_ee_layer(self, ee_image_object, vis_params, name, show=True, opacity=1.0):
    try:
        ...
        tiles_url = (map_id_dict.get('tile_fetcher', {}).url_format
...
# FIX 9: TTL de 1 semana — dados do geobr são estáveis mas não eternos
...
def carregar_areas_protegidas(tipo_area):
    ...
    # FIX 2: fallback robusto caso o geobr mude o nome da coluna
...
def buscar_focos_inpe(tipo, val_estado, val_bioma, val_muni, d_ini, d_fim, satelites):
    ...
    while dt_ini <= dt_fim:
        ...
        cql = (f"data_hora_gmt >= '{dt_ini.strftime('%Y-%m-%d')}T00:00:00' AND "
               f"data_hora_gmt <= '{dt_bloco_fim.strftime('%Y-%m-%d')}T23:59:59' AND "
               f"satelite IN ('{sat_str}') AND pais_complete_id=33 AND {filtro_base}")
        try:
...
if "INPE" in fonte_escolhida:
    ...
    # Variáveis não usadas no MODIS — evita NameError em referências cruzadas
    satelites_sel = []
    unidade_dd = "Meses"
    quantidade_sel = 1
...
# FIX 10: valida satélites ANTES de renderizar o botão — falha rápida, sem processar fronteiras
if "INPE" in fonte_escolhida and not satelites_sel:
    st.sidebar.warning("⚠️ Selecione pelo menos um satélite.")
...
<a href="https://mail.google.com/mail/?view=cm&fs=1&to=anacarolinasantos580@gmail.com" target="_blank"
  ...
  <img src="{gmail_logo_url}" alt="Gmail" style="width: 26px; height: auto;">
</a>
<a href="https://www.linkedin.com/in/ana-carolina-santos-3920931b3" target="_blank"
   style="flex: 1; text-decoration: none; background-color: #0077B5; border-radius: 8px;
          ...
          display: flex; justify-content: center; align-items: center;">
  <img src="{linkedin_logo_url}" alt="LinkedIn" style="width: 22px; height: auto;">
</a>
</div>
</div>
"""
...
if st.session_state.gerar_dashboard:
    ...
    # FIX 10: bloqueia execução se INPE sem satélite (antes de qualquer processamento pesado)
    if "INPE" in fonte_escolhida and not satelites_sel:
        st.error("⚠️ Selecione pelo menos um satélite antes de gerar o dashboard.")
        st.stop()

    hoje = datetime.now()
    val_sel = (bioma_dd if tipo_analise == "Por Bioma"
    ...
    with st.status(f"🛰️ Processando dados para: **{val_sel}**", expanded=True) as status:
        ...
        # FIX 1: unary_union compatível com todas as versões do GeoPandas
        ...
        # FIX 4: inicializa ee_geom_afetadas com o limite completo como fallback
        ...
        if "INPE" in fonte_escolhida:
            ...
            if not df.empty:
                ...
                if area_protegida != "Nenhuma" and not df_rec.empty:
                    ...
                    # FIX 3: errors='ignore' evita KeyError se coluna não existir
            ...
            try:
                ...
                if colecao.size().getInfo() == 0:
                    ...
                    # FIX 7: usa scale=500 em todos os cálculos para consistência com o total
                    ...
                    stats_total = img_area_km2.reduceRegion(
                        reducer=ee.Reducer.sum(), geometry=ee_geom_complex,
                    ...
                    if area_protegida != "Nenhuma" and total_valor > 0:
                        ...
                        # FIX 12: garante coluna nome_area no sjoin do bloco MODIS
                        if 'nome_area' not in gdf_areas_br.columns:
                            gdf_areas_br['nome_area'] = gdf_areas_br.iloc[:, 0].astype(str)

                        # FIX 3: errors='ignore' no drop de index_right
                        gdf_areas = gpd.sjoin(
                        ...
                        if not gdf_areas.empty:
                            ...
                            if not df_ranking_areas.empty:
                                areas_afetadas = gdf_areas[
                                    gdf_areas['nome_area'].isin(df_ranking_areas['Área Protegida'])
                                ]
                                total_valor = round(df_ranking_areas['Valor'].sum(), 2)

                                # FIX 1: unary_union em vez de union_all
                                geom_afetadas = areas_afetadas.geometry.unary_union
                                # FIX 4: atualiza ee_geom_afetadas só quando tiver dados válidos
                                ee_geom_afetadas = ee.Geometry(geom_afetadas.__geo_interface__)
                    ...
                    if tipo_analise != "Por Município" and total_valor > 0:
                        ...
                        # FIX 7: scale=500 consistente com o cálculo de total
                        stats_mun = img_area_km2.reduceRegions(
                            collection=muns_ee, reducer=ee.Reducer.sum(), scale=500
                        ).getInfo()
                        recs_mun = [
                    ...
                    if total_valor > 0:
                        ...
                        # FIX 4: ee_geom_afetadas já está corretamente definida (fallback = ee_geom_complex)
                        ...
                        def calc_mes(m):
                            ...
                            img_mes = (ee.ImageCollection('MODIS/061/MCD64A1')
                                       .filterDate(ini, fim)
                                       .select('BurnDate')
                                       .max()
                                       .clip(geom_temporal))
                            area_calc = ee.Image.pixelArea().divide(1_000_000).updateMask(img_mes.gt(0))
                            # FIX 7: scale=500 para consistência
    ...
    if dados_indisponiveis:
        mes_nome = {1:'Jan',2:'Fev',3:'Mar',4:'Abr',5:'Mai',6:'Jun',
                    7:'Jul',8:'Ago',9:'Set',10:'Out',11:'Nov',12:'Dez'}[mes_modis]
        st.warning(
            f"⏳ **Aviso NASA:** Os dados MODIS para **{mes_nome}/{ano_modis}** ainda não foram publicados. "
            f"Geralmente há atraso de 1–2 meses. Tente um mês anterior."
        )
    elif total_valor == 0:
        # FIX 13: mensagem contextualizada conforme o filtro ativo
        if area_protegida != "Nenhuma":
            st.error(f"⚠️ Nenhum registro detectado dentro das {area_protegida} na região selecionada.")
        else:
            st.error("⚠️ Nenhum registro detectado nos limites selecionados.")
    else:
        texto_titulo = (f"Total Confirmado: {total_valor:,} focos"
                        if "INPE" in fonte_escolhida
                        else f"Área Queimada Total: {total_valor:,.2f} km²")
        texto_sub = (f"Período: {quantidade_sel} {unidade_dd}"
                     if "INPE" in fonte_escolhida
                     else f"Período: Mês {mes_modis} de {ano_modis} (Mapa) / Ano {ano_modis} (Evolução)")

        # Rótulo extra quando filtro de área protegida está ativo
        rotulo_filtro = f" — somente {area_protegida}" if area_protegida != "Nenhuma" else ""
        ...
        if not df_ranking_areas.empty:
            ...
            with col_alerta1:
                ...
                fig_areas = px.bar(
                    df_ranking_areas.head(10), x='Valor', y='Área Protegida',
                    orientation='h', text='Valor', color='Valor',
                    color_continuous_scale=px.colors.sequential.Reds, title=titulo_dinamico
                )
        ...
        with col1:
            ...
            if focar_area != "Visão Geral" and not areas_afetadas.empty:
                ...
                # FIX 1: unary_union
                centro = area_especifica.geometry.unary_union.centroid
            ...
            if not areas_afetadas.empty:
                estilo_tooltip = ("font-size: 12px; max-width: 250px; white-space: normal; "
                ...
                folium.GeoJson(
                    ...
                    tooltip=folium.GeoJsonTooltip(
                        fields=['nome_area'], aliases=['Área Protegida:'], style=estilo_tooltip
                    )
                ).add_to(m)
            ...
            if "INPE" in fonte_escolhida and not df_rec.empty:
                HeatMap(
                    ...
                    radius=15, blur=10, max_zoom=12, min_opacity=0.3
                ).add_to(m)
        ...
        with col2:
            if "INPE" in fonte_escolhida:
                ...
                if not df_rec.empty:
                    # FIX 5: next com fallback evita StopIteration
                    data_col = next((c for c in df_rec.columns if 'data' in c), 'data_hora_gmt')
                    ...
                    fig_line.update_layout(
                        template='plotly_dark', xaxis_title="Tempo",
                        yaxis_title="Nº de Focos", margin=dict(t=20, b=20)
                    )
                    st.plotly_chart(fig_line, use_container_width=True)

                if 'municipio' in df_rec.columns and tipo_analise != "Por Município" and not df_rec.empty:
                    ...
                    fig_bar = px.bar(
                        ...
                        text='Focos', color='Focos', color_continuous_scale=px.colors.sequential.Reds
                    )
                ...
                if tipo_analise != "Por Município" and not df_top_mun_modis.empty:
                    ...
                    fig_bar = px.bar(
                        ...
                        text='Valor', color='Valor', color_continuous_scale=px.colors.sequential.Reds
                    )
                    fig_bar.update_layout(
