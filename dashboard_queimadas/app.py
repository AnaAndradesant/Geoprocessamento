...
if gerar:
    ...
    with st.status(f"🛰️ Processando dados para: **{val_sel}**", expanded=True) as status:
        ...
        if "INPE" in fonte_escolhida:
            ...
            if not df.empty:
                ...
                if area_protegida != "Nenhuma" and not df_rec.empty:
                    ...
                    if not gdf_focos_risco.empty:
                        ...
                        df_rec = pd.DataFrame(); total_valor = 0
        else:
            try:
                ...
                if colecao.size().getInfo() > 0:
                    ...
                    stats_total = img_area_km2.reduceRegion(reducer=ee.Reducer.sum(), geometry=ee_geom_complex, scale=500, maxPixels=1e13, bestEffort=True).getInfo()
                    ...
                    if tipo_analise != "Por Município" and total_valor > 0:
                        ...
                        if recs_mun: df_top_mun_modis = pd.DataFrame(recs_mun).sort_values(by='Valor', ascending=False).head(5)
                    if total_valor > 0:
                        geom_temporal = ee_geom_afetadas if (area_protegida != "Nenhuma" and not areas_afetadas.empty) else ee_geom_complex
                        def calc_mes(m):
                            m_num = ee.Number(m); ini = ee.Date.fromYMD(ano_modis, m_num, 1); fim = ini.advance(1, 'month')
            ...
            except Exception as e: st.warning(f"⚠️ Erro ao processar MODIS: {e}")
    ...
    if total_valor == 0:
        ...
        st.markdown(f"""<div style="background-color: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 8px solid #ff4b4b; margin-bottom: 15px;"><h3 style="color: #c0392b; margin: 0;">🔥 {texto_titulo}</h3></div>""", unsafe_allow_html=True)

        if not df_ranking_areas.empty:
            col_alerta1, col_alerta2 = st.columns([1.5, 1])
            with col_alerta1:
                fig_areas = px.bar(df_ranking_areas.head(10), x='Valor', y='Área Protegida', orientation='h', color='Valor', color_continuous_scale='Reds', title="Top Áreas Afetadas")
                fig_areas.update_layout(template='plotly_dark', height=350); st.plotly_chart(fig_areas, use_container_width=True)
            with col_alerta2:
                st.markdown("**Lista de Áreas Afetadas**"); st.dataframe(df_ranking_areas, hide_index=True, height=350, use_container_width=True)
        ...
        with col1:
            ...
            # 1. CRIAR MAPA BASE COM FUNDO CLARO (CARTODBPOSITRON)
            m = folium.Map(
            ...
            # 2. ADICIONAR CAMADAS ALTERNATIVAS AO MENU (Sem sobrepor o base)
            folium.TileLayer('CartoDB dark_matter', name='Modo Escuro').add_to(m)
            folium.TileLayer(
                tiles='https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}', 
                attr='Google', name='Google Satélite', overlay=False
            ).add_to(m)

            # 3. ADICIONAR DADOS GEOGRÁFICOS
            folium.GeoJson(limite.__geo_interface__, style_function=lambda x: {'fillColor': 'transparent', 'color': '#00d4ff', 'weight': 3}, name="Limites").add_to(m)
            
            if not areas_afetadas.empty:
                folium.GeoJson(areas_afetadas.__geo_interface__, style_function=lambda x: {'fillColor': 'transparent', 'color': '#c0392b', 'weight': 3}, name="Áreas Protegidas").add_to(m)

            # 4. ADICIONAR CALOR OU MODIS
            if "INPE" in fonte_escolhida and not df_rec.empty:
                HeatMap(df_rec[["latitude", "longitude"]].dropna().values.tolist(), radius=15, blur=20, name="Calor").add_to(m)
            ...
            # 5. CONTROLE DE CAMADAS E RENDERIZAÇÃO
        ...
        with col2:
            if "INPE" in fonte_escolhida:
                ...
                fig_line = px.line(df_g, x=data_col, y='focos', height=300); fig_line.update_layout(template='plotly_dark'); st.plotly_chart(fig_line, use_container_width=True)
            else:
                if not df_modis_temporal.empty:
                    st.subheader(f"📈 Evolução ({ano_modis})")
                    fig_modis = px.line(df_modis_temporal, x='Mês Nome', y='Área (km²)', height=300); fig_modis.update_layout(template='plotly_dark'); st.plotly_chart(fig_modis, use_container_width=True)

else:
    st.info("👈 Use os filtros ao lado para selecionar a região e clique em 'Gerar Dashboard'.")
