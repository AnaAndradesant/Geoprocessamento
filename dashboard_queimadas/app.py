...
# INÍCIO DO BLOCO MODIS NA BARRA LATERAL
st.sidebar.markdown("---")
st.sidebar.subheader("🗺️ Cicatrizes de Fogo (NASA MODIS)")
ativar_modis = st.sidebar.checkbox("Ativar camada MODIS", value=False)
if ativar_modis:
    ...
    if df.empty: 
        ...
        if df_rec.empty: 
            ...
            with col1:
                ...
                # --- INÍCIO DA CAMADA MODIS NO MAPA ---
                if ativar_modis:
                    try:
                        st.toast("⏳ Consultando satélite MODIS...")
