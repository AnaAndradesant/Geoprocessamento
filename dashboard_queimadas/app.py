    ...
    if dados_indisponiveis:
        ...
        with aba_graficos:
            if "INPE" in fonte_escolhida:
             ...
             # ----------------------------------------------------------
        # ABA 3 — NBR SENTINEL-2  (VERSÃO CORRIGIDA E COM BOTÃO)
        # ----------------------------------------------------------
        with aba_nbr:
            if "INPE" in fonte_escolhida:
                st.info(
                    "💡 A análise de severidade NBR funciona melhor com a fonte "
                    "**🗺️ Área Queimada (NASA MODIS)**. Selecione MODIS na barra lateral."
                )
            else:
                st.subheader("🔬 Análise de Severidade da Queimada — dNBR (Sentinel-2)")
                st.markdown(
                    "Compara imagens **antes** e **depois** do fogo para medir "
                    "o quanto a vegetação foi danificada."
                )
                    ...
                    if calcular_nbr:
                        with st.spinner("🛰️ Buscando e processando imagens Sentinel-2..."):
                            ...
                            try:
                                ...
                                _, dnbr_img, threshold = _construir_dnbr(
                        ...
                        if nbr_ok and dnbr_img is not None:
                            ...
                            <div style="position:fixed; bottom:30px; right:15px; z-index:9999; 
                                        background:rgba(20,20,20,0.9); padding:12px 16px; 
                                        border-radius:10px; font-size:12.5px; color:white; line-height:1.9;">
                            ...
                            st_folium(m_nbr, width=None, height=650, 
                                      key=f"nbr_map_{val_sel}_{ano_modis}_{mes_modis}")

                # ====================== GRÁFICOS DE SEVERIDADE ======================
                with col_nbr2:
                    if nbr_ok and stats_sev:
                        ...
                        st.markdown(
                            "**Interpretação:**\n\n"
                            "- **Baixa:** vegetação parcialmente afetada\n"
                            "- **Moderada:** danos significativos\n"
                            "- **Alta:** destruição quase total"
                        )
                    else:
